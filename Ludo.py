...
def makeSnake(snakeList, blockSize):
    for xy in snakeList[:-1]:
        pygame.draw.rect(gameWindow, green, (xy[0], xy[1], blockSize,
                         blockSize))
    xy = snakeList[-1]
    pygame.draw.rect(gameWindow, yel, (xy[0], xy[1], blockSize,
                     blockSize))
...
def makeBoarder(sizeApple):
    ...
    pygame.draw.rect(gameWindow, mix, (width - sizeApple, 20, -width,
                     sizeApple))
    pygame.draw.rect(gameWindow, mix, (0, height - sizeApple, width,
                     sizeApple))
    pygame.draw.rect(gameWindow, mix, (width - sizeApple, height
                     - sizeApple, sizeApple, -height + 41))
    pygame.display.update()
...
def GameOver(snakeLength):
    ...
    while gameover is True:
        ...
        message(
            'Press  N to New Game (or) Q to quit!.',
            black,
            y_disp=120,
            size='medium',
            wid=width,
...
def intro():
    ...
    while intro == True:
        ...
        message(
            '1) Eat more Apples and get high score. ',
            blue,
            y_disp=-80,
            size='medium',
            wid=width,
        ...
        message(
            '3) For Pause Game press P ; while playing game ',
            blue,
            y_disp=20,
            size='medium',
            wid=width,
        ...
        message(
            "4) Warning!!! Don't allow snake to strike walls.",
            blue,
            y_disp=70,
            size='medium',
            wid=width,
...
def playGame():
    ...
    yApple = random.randrange(sizeApple + 2 * blockSize, height - 2
                              * sizeApple)
    while gameOver is False:
        ...
        xAxis += xMove
        yAxis += yMove
        if xAxis < sizeApple or xAxis > width - sizeApple - blockSize \
            or yAxis < sizeApple + 2 * blockSize or yAxis > height \
            - sizeApple - blockSize:
            xAxis -= xMove
            yAxis -= yMove
            gameOver = True
    ...
    # pygame.draw.rect(gameWindow,red,(xApple,yApple,sizeApple,sizeApple))
        ...
        if xAxis > xApple and xAxis < xApple + sizeApple or xAxis \
            + blockSize > xApple and xAxis + blockSize < xApple \
            + sizeApple:
            if yAxis > yApple and yAxis < yApple + sizeApple or yAxis \
                + blockSize > yApple and yAxis + blockSize < yApple \
                + sizeApple:
                xApple = random.randrange(sizeApple, width - 2
                        * sizeApple)
